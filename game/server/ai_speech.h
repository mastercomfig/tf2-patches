...
// CAI_Expresser
//
// Purpose: Provides the functionality of going from abstract concept_t ("hello")
...
{
public:
	virtual void OnSpokeConcept( AIConcept_t concept_t, AI_Response *response )	{};
...
public:
	...
	bool Speak( AIConcept_t concept_t, const char *modifiers = NULL, char *pszOutResponseChosen = NULL, size_t bufsize = 0, IRecipientFilter *filter = NULL );

	// These two methods allow looking up a response and dispatching it to be two different steps
	bool SpeakFindResponse( AI_Response &response, AIConcept_t concept_t, const char *modifiers = NULL );
	bool SpeakDispatchResponse( AIConcept_t concept_t, AI_Response &response, IRecipientFilter *filter = NULL );
	...
	bool CanSpeakConcept( AIConcept_t concept_t );
	bool SpokeConcept( AIConcept_t concept_t );
	float GetTimeSpokeConcept( AIConcept_t concept_t ); // returns -1 if never
	void SetSpokeConcept( AIConcept_t concept_t, AI_Response *response, bool bCallback = true );
	void ClearSpokeConcept( AIConcept_t concept_t );
...
private:
	...
	// --------------------------------
	//
	// Speech concept_t data structures
	//
...
public:
	...
	virtual bool 	Speak( AIConcept_t concept_t, const char *modifiers = NULL, char *pszOutResponseChosen = NULL, size_t bufsize = 0, IRecipientFilter *filter = NULL );

	// These two methods allow looking up a response and dispatching it to be two different steps
	bool			SpeakFindResponse( AI_Response& response, AIConcept_t concept_t, const char *modifiers = NULL );
	bool 			SpeakDispatchResponse( AIConcept_t concept_t, AI_Response& response );
	virtual void	PostSpeakDispatchResponse( AIConcept_t concept_t, AI_Response& response ) { return; }
	float 			GetResponseDuration( AI_Response& response );
	...
	void SetSpokeConcept( AIConcept_t concept_t, AI_Response *response, bool bCallback = true ) 		{ this->GetExpresser()->SetSpokeConcept( concept_t, response, bCallback ); }
	float GetTimeSpokeConcept( AIConcept_t concept_t )												{ return this->GetExpresser()->GetTimeSpokeConcept( concept_t ); }
	bool SpokeConcept( AIConcept_t concept_t )														{ return this->GetExpresser()->SpokeConcept( concept_t ); }
...
//-----------------------------------------------------------------------------
template <class BASE_NPC>
inline bool CAI_ExpresserHost<BASE_NPC>::Speak( AIConcept_t concept_t, const char *modifiers /*= NULL*/, char *pszOutResponseChosen /*=NULL*/, size_t bufsize /* = 0 */, IRecipientFilter *filter /* = NULL */ ) 
{
	AssertOnce( this->GetExpresser()->GetOuter() == this );
	return this->GetExpresser()->Speak( concept_t, modifiers, pszOutResponseChosen, bufsize, filter ); 
}
...
//-----------------------------------------------------------------------------
template <class BASE_NPC>
inline bool CAI_ExpresserHost<BASE_NPC>::SpeakFindResponse( AI_Response& response, AIConcept_t concept_t, const char *modifiers /*= NULL*/ )
{
	return this->GetExpresser()->SpeakFindResponse( response, concept_t, modifiers );
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
template <class BASE_NPC>
inline bool CAI_ExpresserHost<BASE_NPC>::SpeakDispatchResponse( AIConcept_t concept_t, AI_Response& response )
{
	if ( this->GetExpresser()->SpeakDispatchResponse( concept_t, response ) )
	{
		PostSpeakDispatchResponse( concept_t, response );
		return true;
	}
