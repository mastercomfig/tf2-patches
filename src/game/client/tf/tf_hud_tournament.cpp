	...
	switch( nTime )
	{
		...
		case 10:
		{
			if ( TFGameRules() && TFGameRules()->IsMannVsMachineMode() )
			{
				...
				{
					if ( ( GTFGCClientSystem()->GetLobby() && IsMannUpGroup( GTFGCClientSystem()->GetLobby()->GetMatchGroup() ) ) || RandomInt(1, 2) > 1 )
					{
...
void CHudTournament::OnTick( void )
{
	...
	if ( TFGameRules() )
	{
		if ( TFGameRules()->IsInTournamentMode() )
		{
			...
			if ( TFGameRules()->IsCompetitiveMode() || (TFGameRules()->UsePlayerReadyStatusMode() && !TFGameRules()->IsMannVsMachineMode()) )
			{
