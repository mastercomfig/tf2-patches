...
#endif
static ConVar tf_movement_stun_legacy_threshold("tf_movement_stun_legacy_threshold", "2.0", FCVAR_REPLICATED, "Relative point for legacy stun amount handling.");
...
bool CTFGameMovement::StunMove()
{
	...
	//
	// A stun value of 0.414 (or 0.2) would have 0% effective stun, 0.6 would have ~20% effective stun, 1.0 would have ~60% effective stun.
	// Given that 0.6 is the maximum stun amount encountered in regular play, we used this as the baseline for legacy handling.
	//
	// This allows us to have linear and consistent scaling for slows from 0.6 down to 0.0 stun amount rather than
	// clipping off to 0 at 0.414, so we essentially buff all slows below 0.6 slightly, and now slows below 0.414 are actually applied.
