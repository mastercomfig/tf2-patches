...
{
	uint32 iMdlCacheTick = vgui::ivgui()->GetMdlCacheTick();

	if ( m_RootMDL.m_pStudioHdr && m_RootMDL.m_nMdlCacheTickCount < iMdlCacheTick )
	{
		m_RootMDL.m_pStudioHdr = new CStudioHdr( m_RootMDL.m_MDL.GetStudioHdr(), g_pMDLCache );
		m_RootMDL.m_nMdlCacheTickCount = iMdlCacheTick;
	}

	const int nMergeCount = m_aMergeMDLs.Count();
	for ( int iMerge = 0; iMerge < nMergeCount; ++iMerge )
	{
		if ( !m_aMergeMDLs[iMerge].m_pStudioHdr || m_aMergeMDLs[iMerge].m_nMdlCacheTickCount >= iMdlCacheTick )
		{
			continue;
		}
		m_aMergeMDLs[iMerge].m_pStudioHdr = new CStudioHdr( m_aMergeMDLs[iMerge].m_MDL.GetStudioHdr(), g_pMDLCache );
		m_aMergeMDLs[iMerge].m_nMdlCacheTickCount = iMdlCacheTick;
	}
}
